...
my ($elem, $remainder);
  ...
  foreach $elem(@{$ref__numbers_list}) {
...
my ($some_number, $simple);
  ...
  return ${$ref__numbers_list}[$#{$ref__numbers_list}];
}
...
my @numbers_list = ();
my $start_time = time();
my $last_number = fill_simple_numbers_list($program_params{target_numbers_count}, \@numbers_list);
my $stop_time = time();
